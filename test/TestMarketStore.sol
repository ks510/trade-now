...
contract TestMarketStore {
  ...
  function testMarketStoreInitializedCorrectly() public {
    ...
    uint initialListingCount = 0;
    uint totalListings = market.totalListings();
    Assert.equal(totalListings, initialListingCount, "Market store should start with 0 listings");
  }

  // test creating a new listing is stored correctly and can be retrieved
  function testCreateAndRetrieveListing() public {
    ...
    (listingId,
      ...
      listingImage,
      listingSeller,
      listingState) = market.getListing(generatedId);
    ...
    Assert.equal(listingState, 0, "Listing state was not initialized correctly - expected AVAILABLE");

  }

  function testCreateAndRetrieveListingPart2() public {
    MarketStore market = MarketStore(DeployedAddresses.MarketStore());
    // separated due to stack too deep error (too many local variables in above method)
  ...
  // check listing ids are unique and generated correctly
  function testListingIdGeneration() public {
    MarketStore market = MarketStore(DeployedAddresses.MarketStore());

    // create second listing
    uint generatedId2 = market.createListingInStore(200, "Leather Coat", "Mint condition", "", address(this));
    Assert.equal(generatedId2, 2, "The second listing should have id = 2");

    // create third listing
    uint generatedId3 = market.createListingInStore(300, "Sunglasses", "Worn a few times", "", address(this));
